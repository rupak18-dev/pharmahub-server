...
export async function refreshBatchStatus(
  batch,
  nearExpiryDays = constants.expiry.nearExpiryDays,
) {
  const expiryDate = batch.dates?.expiryDate;

  if (!expiryDate) return batch;

  batch.status = classifyBatchStatus(expiryDate, nearExpiryDays);
...
export async function getNearExpiryBatches(
  days = constants.expiry.nearExpiryDays,
) {
...
export async function getLowStockMedicines() {
  ...
  for (const b of batches) {
    ...
    perMedicine.set(
      key,
      (perMedicine.get(key) ?? 0) +
        (b.stock?.quantityOnHand ?? 0),
    );
  }
  ...
  return medicines
    .filter(
      (m) =>
        (perMedicine.get(String(m._id)) ?? 0) <=
        m.reorderThreshold,
    )
    .map((m) => ({
      medicine: m,
